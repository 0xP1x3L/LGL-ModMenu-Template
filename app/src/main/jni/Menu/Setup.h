...
void setDialog(jobject ctx, JNIEnv *env, const char *title, const char *msg) {
  ...
  jmethodID AlertCons = env->GetMethodID(
      Alert, OBFUSCATE("<init>"), OBFUSCATE("(Landroid/content/Context;)V"));

  jobject MainAlert = env->NewObject(Alert, AlertCons, ctx);

  jmethodID setTitle = env->GetMethodID(
      Alert, OBFUSCATE("setTitle"),
  ...
  jmethodID setMsg = env->GetMethodID(
      Alert, OBFUSCATE("setMessage"),
      OBFUSCATE("(Ljava/lang/CharSequence;)Landroid/app/AlertDialog$Builder;"));
  env->CallObjectMethod(MainAlert, setMsg, env->NewStringUTF(msg));

  jmethodID setCa =
      env->GetMethodID(Alert, OBFUSCATE("setCancelable"),
                       OBFUSCATE("(Z)Landroid/app/AlertDialog$Builder;"));
  env->CallObjectMethod(MainAlert, setCa, false);

  jmethodID setPB =
      env->GetMethodID(Alert, OBFUSCATE("setPositiveButton"),
                       OBFUSCATE("(Ljava/lang/CharSequence;Landroid/content/"
                                 "DialogInterface$OnClickListener;)Landroid/"
                                 "app/AlertDialog$Builder;"));
  env->CallObjectMethod(MainAlert, setPB, env->NewStringUTF("Ok"),
  ...
  jmethodID show =
      env->GetMethodID(AlertN, OBFUSCATE("show"), OBFUSCATE("()V"));
...
void Toast(JNIEnv *env, jobject thiz, const char *text, int length) {
  ...
  jmethodID methodMakeText = env->GetStaticMethodID(
      toast, OBFUSCATE("makeText"),
      OBFUSCATE("(Landroid/content/Context;Ljava/lang/CharSequence;I)Landroid/"
                "widget/Toast;"));
  jobject toastobj =
      env->CallStaticObjectMethod(toast, methodMakeText, thiz, jstr, length);
  jmethodID methodShow =
      env->GetMethodID(toast, OBFUSCATE("show"), OBFUSCATE("()V"));
...
void startActivityPermisson(JNIEnv *env, jobject ctx) {
  ...
  jmethodID startActivity =
      env->GetMethodID(native_context, OBFUSCATE("startActivity"),
  ...
  jmethodID Parce = env->GetStaticMethodID(
      Uri, OBFUSCATE("parse"),
      OBFUSCATE("(Ljava/lang/String;)Landroid/net/Uri;"));
  jobject UriMethod =
      env->CallStaticObjectMethod(Uri, Parce, env->NewStringUTF(pakg.c_str()));

  jclass intentclass = env->FindClass(OBFUSCATE("android/content/Intent"));
  jmethodID newIntent =
      env->GetMethodID(intentclass, OBFUSCATE("<init>"),
                       OBFUSCATE("(Ljava/lang/String;Landroid/net/Uri;)V"));
  jobject intent =
      env->NewObject(intentclass, newIntent,
...
void startService(JNIEnv *env, jobject ctx) {
  ...
  jclass intentClass = env->FindClass(OBFUSCATE("android/content/Intent"));
  jclass actionString =
      env->FindClass(OBFUSCATE("com/android/support/Launcher"));
  jmethodID newIntent = env->GetMethodID(
      intentClass, OBFUSCATE("<init>"),
  ...
  if (sdkVer >=
      26) { // Android 8.0 (Oreo) and above requires startForegroundService
    jmethodID startFgServiceMethodId = env->GetMethodID(
        native_context, OBFUSCATE("startForegroundService"),
        OBFUSCATE("(Landroid/content/Intent;)Landroid/content/ComponentName;"));
    env->CallObjectMethod(ctx, startFgServiceMethodId, intent);
  } else {
    jmethodID startServiceMethodId = env->GetMethodID(
        native_context, OBFUSCATE("startService"),
        OBFUSCATE("(Landroid/content/Intent;)Landroid/content/ComponentName;"));
...
void CheckOverlayPermission(JNIEnv *env, jclass thiz, jobject ctx) {
  // If overlay permission option is greyed out, make sure to add
  ...
  if (sdkVer >= 23) { // Android 6.0
    ...
    jmethodID canDraw =
        env->GetStaticMethodID(Settings, OBFUSCATE("canDrawOverlays"),
...
void Init(JNIEnv *env, jobject thiz, jobject ctx, jobject title,
  ...
  setText(env, subtitle,
          OBFUSCATE(
              "<b><marquee><p style=\"font-size:30\">"
